...
We emphasize that \ours{} targets \emph{distributional anomaly detection} in unlabeled surveillance streams, not taxonomic classification of specific variants. Discriminating between highly similar variants—such as Omicron and wildtype SARS-CoV-2, which differ at approximately 50 of 30,000 genome positions—at the read level requires either supervised lineage labels or pathogens with substantially greater inter-strain diversity; neither is available in the reference-free surveillance setting this work addresses. With 150\,bp short reads, the expected number of variant-defining positions spanned per read is $50 \times 150 / 30{,}000 \approx 0.25$, meaning roughly 78\% of reads are genomically indistinguishable between lineages at the $k$-mer level—a fundamental constraint that applies equally to all read-level representation methods, supervised or otherwise. \ours{}'s value is in flagging when the \emph{population-level} genomic composition of a wastewater sample diverges from a known baseline, triggering targeted downstream sequencing of samples most likely to contain novel variants.
...
\begin{enumerate}
    ...
    \item \textbf{Anomaly detection.} AUROC and AUPRC using distance-to-nearest-centroid as the anomaly score, with the smallest $K$-means cluster designated as the anomaly class. We additionally validate this protocol with a positive-control injection experiment: Omicron reads are injected at controlled rates (1\%–50\%) into a Wuhan baseline stream, providing explicit positive labels for AUROC evaluation (Section~\ref{sec:lineage_separation}).
...
To characterize the boundaries of what read-level embeddings can achieve, we conducted two experiments using 10,000 simulated 150\,bp reads from each of Wuhan-Hu-1 (NC\_045512.2), Delta (B.1.617.2), and Omicron BA.1 reference genomes (uniform sliding-window simulation, 0.5\% error rate).

\paragraph{Cluster separation.}
Table~\ref{tab:lineage_sep} reports lineage-separation metrics for all three models. All metrics are consistent with random performance across all models.

\begin{table}[t]
  \centering
  \caption{Lineage separation on simulated reads (10K reads/lineage). All metrics are near chance, consistent with the biological constraint that $\sim$78\% of 150\,bp reads share no variant-defining $k$-mer between Wuhan and Omicron.}
...
We further validated the anomaly detection protocol by injecting Omicron reads at controlled rates into a Wuhan reference stream and evaluating with explicit positive labels. Table~\ref{tab:injection} reports results for \ours{}; Contrastive VQ-VAE variants show identical trends.

\begin{table}[t]
  \centering
  \caption{Positive-control injection experiment (\ours{}). Omicron reads injected into a Wuhan baseline stream at target rates 1\%–50\%. AUROC and AUPRC remain at chance across all rates, confirming that the near-random performance is a consequence of genomic conservation rather than a model artifact.}
  \label{tab:injection}
  \small
  \begin{tabular}{lcccc}
  \toprule
...
These results are the expected consequence of SARS-CoV-2's genomic conservation, not a failure of the representation learning approach. With $\sim$50 substitutions across 30,000 positions, the expected number of variant-defining sites spanned by a 150\,bp read is $50 \times 150 / 30{,}000 \approx 0.25$. Each such substitution alters at most 6 of $\sim$145 6-mer tokens ($\sim$4\% of tokens per read), providing insufficient signal for an unsupervised model—or any read-level method—to score injected reads above random. The same constraint would apply to any approach, including supervised ones, that operates at the individual-read level on a pathogen of this sequence conservation. \ours{}'s operational value is in detecting population-level compositional shifts across many reads from a wastewater sample, not in assigning individual reads to named lineages.
...
\ours{} is designed to detect when the genomic composition of a wastewater sample diverges from a known baseline—not to assign individual reads to named clades. As Section~\ref{sec:lineage_separation} demonstrates, read-level variant discrimination between highly similar lineages (e.g., Wuhan vs.\ Omicron, $\sim$99.97\% identical) yields near-chance AUROC ($0.50 \pm 0.02$) regardless of model or injection rate. This is a fundamental biological constraint, not a modelling failure: the correct response is to operate at the sample level, where distributional shift across thousands of reads provides a detectable signal even when individual reads are indistinguishable. Generalization to pathogens with greater inter-strain diversity—influenza, RSV, norovirus—is a natural next step and would provide more informative lineage-discrimination benchmarks.

\paragraph{Implications and limitations.}
Reference-free representations address a key WBE gap: novel clusters can trigger targeted sequencing without requiring prior variant definitions. The compact 64-dimensional embeddings provide a 12$\times$ storage reduction over DNABERT-2 (256 bytes vs.\ 3,072 bytes per read), operationally significant at the millions-of-reads-per-sample scale of wastewater monitoring. Limitations: (1) the anomaly detection protocol uses $K$-means pseudo-labels and distance-to-centroid scoring, both of which operate at the sample level—read-level discrimination of similar variants is not achievable without supervised labels, as confirmed by the injection experiment; (2) training is on SARS-CoV-2 from one region, requiring validation on other pathogens and sequencing platforms; (3) embeddings use mean-pooled pre-quantization representations rather than discrete codes directly—leveraging codebook structure for retrieval and anomaly scoring remains an open direction.
...
We designate the smallest $K$-means cluster as the anomaly class and use the distance from each test embedding to its nearest cluster centroid as the anomaly score. We report AUROC and AUPRC. High scores indicate that the embedding space places rare or outlier sequences at greater distances from cluster centers, enabling detection of potentially novel variant lineages. The positive-control injection experiment (Section~\ref{sec:lineage_separation}) provides explicit positive labels to validate this protocol under controlled conditions.
