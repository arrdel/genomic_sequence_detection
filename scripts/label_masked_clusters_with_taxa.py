#!/usr/bin/env python3
"""
Label Masked VQ-VAE Clusters with Taxonomy (Genus Level)

This script takes the cluster assignments from masked VQ-VAE evaluation and 
...
Usage:
    python scripts/label_masked_clusters_with_taxa.py \
      --checkpoint-path experiments/2_masked_vqvae/checkpoints/best_model.pt \
      ...
      --output-dir experiments/2_masked_vqvae/results_labeled \
      --k-mer 6 \
...
def extract_embeddings_and_cluster(model, dataloader, device, num_clusters=10):
    """Extract embeddings from masked VQ-VAE and cluster them"""
    model.eval()
    all_embeddings = []
    
    print("\nExtracting embeddings from Masked VQ-VAE...")
...
def plot_umap_labeled(embeddings, cluster_labels, cluster_info, out_png, figsize=(14, 10)):
    ...
    ax.set_title("Masked VQ-VAE: UMAP with Genus-Level Labels", fontsize=14, fontweight='bold')
...
def plot_tsne_labeled(embeddings, cluster_labels, cluster_info, out_png, figsize=(14, 10)):
    ...
    ax.set_title("Masked VQ-VAE: t-SNE with Genus-Level Labels", fontsize=14, fontweight='bold')
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=9, framealpha=0.9)
    ax.grid(alpha=0.3)
...
def plot_cluster_genus_composition(cluster_info, out_png, figsize=(14, 10)):
    ...
    plt.suptitle('Genus-Level Composition of Masked VQ-VAE Clusters', 
...
def main():
    parser = argparse.ArgumentParser(description='Label Masked VQ-VAE clusters with genus-level taxonomy')
    ...
    print("\n" + "="*80)
    print("MASKED VQ-VAE: GENUS-LEVEL TAXONOMIC LABELING")
    print("="*80)
    ...
    # Load model
    print("\n" + "="*80)
    print("LOADING MASKED VQ-VAE MODEL")
    print("="*80)
    ...
    if 'args' in checkpoint:
        ...
        vocab_size = model_args.get('vocab_size', 4099)
